...
class CustomEfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, in_channels=3, n_grapheme=168, n_vowel=11, n_consonant=7):
        ...
        # consonant_diacritic
        self.logits_for_consonant = nn.Linear(out_channels, n_consonant)
        # grapheme_root
        self.logits_for_vowel = nn.Linear(out_channels, n_vowel)
    ...
    def forward(self, inputs):
        ...
        x = self._dropout(x)
        # target_col = ['grapheme_root', 'consonant_diacritic', 'vowel_diacritic']
        logits_for_grapheme = self.logits_for_grapheme(x)
        logits_for_consonant = self.logits_for_consonant(x)
        logits_for_vowel = self.logits_for_vowel(x)
